...
export default function SearchPage() {
  ...
  return (
    <>
      <SiteNavbar />
      <main className="pt-20 md:pt-24 h-screen flex flex-col overflow-hidden">
        <div className="max-w-7xl w-full mx-auto px-4 md:px-6 pt-4 pb-3 shrink-0">
          <PageSearchBar />
        </div>
        ...
        <div className="flex-1 min-h-0 max-w-7xl w-full mx-auto px-4 md:px-6 pb-4 grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_1.25fr] grid-rows-[minmax(0,1fr)] gap-4 md:gap-6">
          {/* Results column */}
          <section
            className={`${view === "list" ? "flex" : "hidden"} md:flex flex-col min-h-0 h-full`}
          >
            <div className="flex items-center justify-between mb-3 shrink-0">
              <h1 className="font-headline font-bold text-lg md:text-xl text-on-surface">
                {results.length} results near London
              </h1>
              <button className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface-container-lowest border border-on-surface/[0.06] hover:bg-primary-fixed/40 transition-colors">
